...
무엇에 쓰는지는 아래 *회의와 세미나*에 있습니다. 서버가 직접 받아 오는 방송에서는
기본값이 끔이고, 드는 비용은 실측이 아니라 산술입니다. 표본이 16kHz 모노
16비트이므로 16000 × 2 = 초당 32KB — 한 시간에 115MB쯤, 일곱 시간 방송이면
800MB쯤이고, 멀티뷰는 그런 세션을 넷까지 돌립니다. 저절로 지워지지 않습니다.

마이크·탭 세션은 이 칸과 무관하게 소리를 저장합니다. 거기서는 녹음이 세션을 켠
이유이고 소리가 다른 어디에도 없으니, 칸을 비워 둔다고 그것이 꺼지지는 않습니다.
이 칸은 서버가 직접 받아 오는 방송에 저장을 붙이는 칸입니다.
...
시작합니다. 브라우저가 마이크 권한을 묻고, 그 뒤로 두 가지가 동시에
일어납니다. 자막이 실시간 미리보기로 흐르고, **소리는
...
| `data/recordings/` | 세션이 쓴 WAV — 마이크·탭 세션은 언제나, 서버가 받아 오는 방송은 **소리를 파일로 저장**을 켰을 때. 한 시간에 115MB쯤입니다(16kHz × 2바이트 = 초당 32KB). 자동으로 지우지 않습니다 — 다시 만들 수 없는 유일한 산출물입니다 |
