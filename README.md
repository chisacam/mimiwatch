...
스크립트**입니다. 양옆은 각각 접힙니다 — 목록은 `V`, 스크립트는 `S`.
...
id로 저장되므로 그때까지의 스크립트가 사라졌습니다. 한 영상 안에서 자막은
이어져야 합니다.
...
움직입니다. 스크립트 패널만 상자 밖이라 보이지 않습니다.
...
멈춰 있었으면 못 메운 초를 스크립트에 한 줄로 적습니다.
...
자막이 **재생 위치를 따라갑니다** — 스크립트에서 줄을 눌러 그 자리로 가면
거기서부터 자막이 뜨고, 오른쪽 스크립트도 지금 줄을 가운데에 맞춥니다.
...
스크립트 머리 아래에 **모드**가 있습니다. 줄을 눌렀을 때 무슨 일이 일어날지를
정합니다.
...
읽기 모드에서는 편집 단추가 아예 보이지 않습니다. 스크립트의 본래 일은 읽는
...
라이브를 받는 중에 고치면 대본 창에도 바로 반영됩니다.
...
라이브를 받는 중에도 됩니다. 다시 번역한 줄은 대본 창에도 바로 닿습니다.

### 내보내기

스크립트 머리의 **⤓ 내보내기** 로 쌓인 자막을 파일로 받습니다. 녹화본과
라이브 모두 됩니다.
...
시작하면 **대본 창이 저절로 열립니다.** 이 흐름에는 끼워 넣을 영상이 없어서
...
뜹니다 — 누르면 탭을 다시 공유하라고 묻고, 그 뒤부터 같은 스크립트에 번호와
...
임베드가 막힌 방송을 위해, 대본만 담은 별도 창을 띄울 수 있습니다. 스크립트
머리의 **⧉ 대본 창** 을 누르면 세로로 긴 창이 하나 열립니다. 유튜브를 한쪽에
띄워 두고 이 창을 옆에 붙여 두는 용도입니다.

이 창에는 목록도 플레이어도 없고 대본만 있습니다. 자막은 본 창과 똑같이
...
| 도구 | 하는 일 |
| --- | --- |
| 둘 다 / 번역만 / 원문만 | 대본 줄에 무엇을 보일지 |
| 글자 크기 | 11~24px |

선택은 기억되며, 본 창의 스크립트에도 같이 적용됩니다.
...
**「대본을 채팅 자리에」** 를 켜면 오른쪽 열에 대본이 섭니다. 라이브면 채팅을
...
목록 관리·엔진 설정·스크립트 편집은 팝업에 없습니다. mimiwatch 페이지가
...
**대본을 액자로 끼우지 않았습니다.** 이미 만들어 둔 `?script=…` 화면을 그대로
