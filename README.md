...
1. 가상환경을 만들고 의존성을 설치합니다 (전사 런타임 휠 포함)
2. 모델을 내려받습니다 — 화면의 「모델·도구」와 같은 목록(`modelhub.py`)입니다
3. `backends.example.json`을 복사해 `backends.json`을 만듭니다

**이미 끝난 단계는 건너뜁니다.** 내려받기가 중간에 끊기면 그냥 다시
실행하십시오 — 받다 만 것(`.part`)은 이어 받습니다.

### 기본은 가벼운 CPU 엔진입니다 — 첫 실행에서 고릅니다

사양은 기계마다 다릅니다. 그래서 **기본 설정은 어디서든 도는 가벼운 CPU
엔진**(전사 SenseVoice Small, 번역 M2M-100 — hayamimi와 같은 조합)이고, 설치
스크립트도 그 조합에 필요한 것만 받습니다(약 730MB). 품질은 무거운 쪽이
훨씬 낫습니다 — GPU(Metal·Vulkan)가 있고 메모리가 넉넉하면 올리십시오.

화면을 처음 열면 **「초기 설정」** 대화상자가 떠서 전사·번역 엔진을 고르게
하고, 고른 조합에 필요한 모델만 그때 받습니다. 「관리 › ⚙ 엔진 관리 › 초기
설정 다시」로 언제든 다시 고를 수 있습니다. 설치할 때부터 Gemma까지 받아
두려면:
...
| 모델 | 크기 | 쓰임 |
|---|---|---|
| SenseVoice Small Q8_0 | 241MB | 전사 · **기본** (가벼움 · CPU) |
| whisper-large-v3-turbo Q8_0 | 845MB | 전사 · 품질 (모든 언어 · GPU 권장) |
| Moonshine base Q8_0 | 74MB | 가벼운 전사 (영어 전용) |
| M2M-100 (CTranslate2) | 473MB | 번역 · **기본** (가벼움 · CPU · Gemma의 대체용도 겸함) |
| Gemma 4 E4B q4_0 | 4.9GB | 번역 · 품질 (GPU 권장) |
| CAM++ | 27MB | 녹화본 화자 태그 |
...
「관리 › ⚙ 엔진 관리」의 **모델·도구** 구역에 위 표가 그대로 있습니다 —
무엇이 있고 없는지, 받다 만 것은 얼마나 받았는지, 지울 것은 무엇인지.
필수 모델이 없으면 화면 위쪽에 띠가 떠서 「초기 설정」에서 고른 조합을
받습니다. 내려받기는 서버가 배경에서 하고 진행은 화면에 그대로 흐릅니다.
끊기면 다시 눌러 이어 받습니다.

**허깅페이스에서 직접 받을 수도 있습니다.** 「＋ 허깅페이스에서 추가」에
저장소(`소유자/이름`)와 `.gguf` 파일 이름을 적으면 받아서 전사(transcribe.cpp)
또는 번역(llama.cpp) 엔진으로 등록합니다 — 바로 「전사」·「번역」 선택기에
나타납니다. 라이선스 동의가 필요한 저장소는 토큰 칸에 넣거나 `HF_TOKEN`
환경변수를 주십시오(저장하지 않습니다). 그 모델이 정말 transcribe.cpp나
llama.cpp가 읽는 것인지는 받은 뒤 처음 쓸 때 드러납니다.

`ffmpeg`와 `yt-dlp` 독립 실행 파일도 같은 자리에서 받습니다. 시스템에
있으면 그렇게 표시되고 받을 필요가 없습니다.

명령줄로도 됩니다. 설치 스크립트가 쓰는 것과 같습니다.

```sh
.venv/bin/python modelhub.py list
.venv/bin/python modelhub.py download default --skip-gemma
.venv/bin/python modelhub.py download sensevoice-small ffmpeg
```
...
**멈춘 방송은 「▶ 이어받기」로 같은 세션에 이어 붙입니다.** 고르개에서 멈춘
방송(「· 멈춤」)을 고르면 살아납니다. 서버가 죽었든 사용자가 중단했든 같고,
탭 소리로 받던 세션이면 이 탭의 소리를 다시 잡습니다. 끝난 방송(「· 끝난
방송」)은 이어받을 것이 없으니 전체 영상 전사는 mimiwatch 페이지에서 합니다.
...
**듣는 길과 받아 적는 길은 나뉘어 있습니다.** 되돌려 주는 소리는 잡은
스트림을 `<audio>` 에 그대로 붙인 것이라 표본율도 채널도 원본 그대로입니다.
받아 적는 쪽만 16kHz 모노로 접습니다 — 인식기가 원하는 모양이지 사람이 들을
모양이 아닙니다. 한동안 되돌려 주는 소리까지 그 16kHz 그래프를 지나가서
48kHz 스테레오가 전화 음질로 깎여 나갔습니다.

접는 일은 Web Audio 에 맡깁니다(`channelCount:1`, `channelInterpretation:
"speakers"`). 손으로 왼쪽 채널만 집으면 오른쪽에 치우친 목소리를 통째로
놓칩니다 — 오른쪽에만 0.8 을 넣고 재어 보면 접기로는 0.4 가 살아오고,
왼쪽만 집으면 **0** 입니다.
...
MIT를 고르지 않은 이유는 그 한 가지입니다. MIT는 상업적 이용까지 열어 두는데,
이 프로젝트는 그렇지 않기를 바랍니다. 참고한 hayamimi(MIT)에서 가져온 라이브 전사
루프의 구조에는 그쪽 고지가 `LICENSE` 안에 그대로 남아 있습니다.

모델과 ffmpeg은 저장소에 담기지 않고 사용자가 직접 내려받으며 각자의 라이선스
(예: Gemma 이용 약관)를 따릅니다.
