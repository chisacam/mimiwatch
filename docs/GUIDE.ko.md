...
`data/recordings/<날짜>-<세션id>.mp4`로 복사합니다. 그림은 온 그대로 복사하고
소리만 다시 인코딩하므로 드는 것은 디스크이고 CPU가 아닙니다. 서버가 받아 오는
방송에만 해당합니다. 마이크·탭
세션은 소리만 올려 보내 저장할 그림이 없으므로 그 소스에서는 칸이 숨습니다.
소리 쪽과 마찬가지로 켜지 않으면 저장하지 않습니다.
...
**두 번째 출력이 아니라 두 번째 프로세스입니다.** 녹화기는 자기 몫의 ffmpeg이고,
자기 몫의 muxed 화질을 따로 받아 옵니다. 전사 파이프라인은 일부러 *소리만 있는*
화질을 고르므로 거기에 영상을 태우면 픽셀을 받아다 그대로 버리는 일이 되고, 둘이
함께 재접속하게 됩니다. 지금은 한쪽이 죽어도 다른 쪽은 그대로입니다. 자막은 칸을
껐을 때와 똑같이 나옵니다.
...
안에 만료되므로, 방송이 이어지는 중에 녹화기의 ffmpeg이 끝납니다. 그러면 다시
받아 와서 첫 파일 옆에 다음 조각을 엽니다 — `… (2).mp4`, `… (3).mp4`. 소리 프레임을
이어 붙이듯 복사본을 파일 중간부터 이어 쓸 수는 없기 때문입니다. 시작하자마자
죽는 일이 다섯 번이면 포기하고 그 사실을 적습니다. 30초를 버틴 조각은 그 횟수를
되돌립니다. **이어받기**는 두 번째 WAV를 열듯이, 자기 시각으로 자기 조각들을
엽니다.
...
**실패해도 다른 것을 잃지 않습니다.** 화질을 받아 오지 못하거나, ffmpeg을 세우지
못하거나, 끝내 죽으면, WAV가 실패했을 때처럼 그 이유가 상태 줄에 뜨고(*영상 저장이
멈췄습니다*) 세션은 전사와 번역과 소리 저장을 계속합니다. 멀티뷰는 세션을 넷까지
돌리므로 영상 녹화기도 넷까지 함께 돕니다 — 소리 넷 위에 muxed 스트림 넷을 더
받아 쓰는 셈입니다.
