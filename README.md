...
다만 **자막은 그대로 동작합니다.** 오디오는 서버가 쿠키로 따로 받아 적으므로,
유튜브를 다른 창에 띄워 두고 오른쪽 스크립트를 읽으면 됩니다. 화면 위 자막을
쓰려면 오프셋 슬라이더로 맞추십시오 — 자동 정렬은 임베드된 플레이어의 재생
위치를 읽어야 하는데 그 플레이어가 없기 때문입니다.
