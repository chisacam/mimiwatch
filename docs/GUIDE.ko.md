...
README 에서 옮겨 온 상세 안내입니다. 설치와 실행은 [README](../README.ko.md) 에,
...
**왼쪽 「＋ 추가」**에 YouTube 주소나 Twitch 채널 주소, 치지직 주소, m3u8
주소를 넣습니다. 라이브인지 녹화본인지는 서버가 판단하므로 고르실 필요가
없습니다. 생 m3u8 은
...
아닙니다. 제 그림을 들고 온 줄(치지직 녹화본)은 그것을 씁니다. m3u8을 직접
넣은 세션에는 영상 id가 없어 자리만 비어 있습니다.
...
**목표 언어는 기계에 저장됩니다.** 「관리」의 「내 언어」가 자막이 어떤 언어로
나오는지 정합니다. 마지막 선택은 서버(설정 파일)에 저장되어 mimiwatch
페이지와 브라우저 확장지가 하나의 값을 공유합니다. 한쪽에서 정한 기본값이
다른 쪽으로 이어지고, 새 브라우저 프로필에서도 살아 남습니다.

### 치지직

치지직 주소도 같은 칸에 넣습니다 — 방송(`chzzk.naver.com/live/…`)이든
녹화본(`chzzk.naver.com/video/…`)이든.

**화면은 치지직이 주는 매니페스트로 틉니다.** 치지직은 임베드 플레이어를
내주지 않으므로, 페이지가 서버가 풀어 준 주소를 직접 틉니다 — 방송과 방송의
다시보기는 hls.js로, 파일 하나로 주는 녹화본은 맨 video 요소로. 그 주소는
서명되어 하루 안에 만료되므로, 줄을 열 때 받아 오고 저장하지 않습니다 —
들고 있던 주소는 403을 냅니다.

**녹화본은 yt-dlp를 타지 않습니다.** 상당수는 yt-dlp가 치지직의 매니페스트를
아예 읽지 못하므로, 서버가 치지직의 엔드포인트에 직접 묻습니다(`chzzk.py`,
실측은 [RESULTS 53절](../measurements/RESULTS.ko.md)). 같은 답에 녹화본 제
썸네일이 함께 오고, 전사에는 가장 싼 렌디션을 가져옵니다 — 소리는 어느
렌디션이나 같고 화면은 다르기 때문입니다(일곱 시간 방송에서 26GB 대 565MB).

**구독자 전용·연령 제한 녹화본은 로그인이 필요합니다.** 치지직 쿠키는
브라우저에서 읽지 않고 엔진 판에 붙여 넣습니다 — 로그인이 치지직이 아니라
네이버의 것이기 때문이고, 유튜브 쿠키와 따로 보관되어 서로를 덮어쓰지
않습니다. 공개 방송과 공개 녹화본에는 아무것도 필요하지 않습니다.
...
**소리를 파일로 저장** — 세션이 받은 소리를 `data/recordings/<날짜>-<세션id>.wav`에
씁니다. 나중에 더 나은 전사기로, 또는 화자 구분을 붙여 다시 전사할 수 있습니다.
무엇에 쓰는지는 아래 *회의와 세미나*에 있습니다. 드는 비용은 실측이 아니라
산술입니다. 표본이 16kHz 모노 16비트이므로 16000 × 2 = 초당 32KB — 한 시간에
115MB쯤, 일곱 시간 방송이면 800MB쯤이고, 멀티뷰는 그런 세션을 넷까지 돌립니다.
저절로 지워지지 않습니다.

**이 칸을 켜지 않으면 아무것도 남지 않습니다.** 마이크든 다른 탭이든 서버가 받아
오는 방송이든 마찬가지입니다. 전에는 마이크·탭 소스에서 저절로 켜져 있었습니다.
회의는 녹음하려고 켜는 것이라는 이유였는데, 페이지는 칸을 켰을 때만 그 항목을
보냈으므로 칸을 끄는 일에는 아무 효과가 없었습니다. 켜기만 되는 칸은 설정이
아니고, 아무도 요청하지 않은 녹음은 디스크에 남을 것이 아닙니다.

**영상도 파일로 저장** — 방송 자체를 소리와 나란히
`data/recordings/<날짜>-<세션id>.mp4`로 씁니다. 켜면 방송을 **한 번만 받아 와서
갈라 씁니다** — 한 번 읽은 것이 자막과 WAV와 mp4를 함께 먹입니다. 그림은 온
그대로 복사하고 소리만 다시 인코딩하므로 드는 것은 디스크이고 CPU가 아닙니다.
서버가 받아 오는 방송에만 해당합니다. 마이크·탭 세션은 소리만 올려 보내 저장할
그림이 없으므로 그 소스에서는 칸이 숨습니다. 소리 쪽과 마찬가지로 켜지 않으면
저장하지 않고, 켜든 끄든 수신 쪽은 아무것도 달라지지 않습니다 — 소리는 어느
쪽이든 같은, 소리만 있는 화질에서 읽습니다.

여기에는 한 시간에 몇 MB라고 적을 숫자가 없습니다. 소리 쪽 수치는 바뀌지 않는
표본 형식에서 나온 산술이지만, 영상 화질에는 고정된 것이 없습니다 — 사이트가
무엇을 내주느냐가 크기를 정합니다. 그러니 처음 한 번은 디렉터리를 직접 보는 것이
정직한 안내입니다. 나머지는 아래 *서버가 받아 오는 방송을 저장하기*에 있습니다.

**어느 쪽 답도 최종이 아닙니다.** 두 칸은 수신 중인 세션의 상태 줄에 스위치로
다시 나옵니다. 남길 만한 방송이라는 것을 도중에 알게 되어도 세션을 멈췄다 다시
시작할 필요 없이 그 자리에서 켤 수 있습니다. 그 값이 얼마인지는 — 소리 쪽은
아무것도 아니고 영상 쪽은 몇 초짜리 재접속입니다 — 아래
*방송이 도는 중에 저장을 바꾸기*에 있습니다.
...
옆에 다른 라이브를 붙입니다 — YouTube, Twitch, 치지직, m3u8 어느 것이든,
넷까지.
...
(서버가 그렇게만 열립니다). 생 m3u8 과 치지직은 hls.js 로 틉니다 — 방송 서버가 다른
출처의 접근(CORS)을 막으면 화면은 열리지 않지만, 자막은 서버가 ffmpeg 으로
받아 적으므로 계속 쌓입니다. (치지직의 미디어 서버는 허용하고 있고, 그래서
우리 페이지에서 틀 수 있습니다.)
...
영상 목록에 검색 상자가 있습니다. **제목이 아니라 자막 텍스트** — 원문 줄과 번역 양쪽 — 을
전체 도서관에서 찾습니다. 단어를 쓰면 목록이 맞는 줄로 바뀝니다. 각 줄에 영상 이름과
시각이 붙고, 줄을 누르면 그 영상으로 열어 그 자리로 이동합니다.

검색은 단어 단위입니다. 쓴 단어가 전부 줄 안에 있어야(모두를 가진 줄이) 매칭되고, 표시된
구간이 그 자립니다. 반쯤 기억나는 줄을 찾는 데 쓰면 됩니다 — "미미라고 한 그 줄" 같은 것.
...
내보내기 대화상자의 **영상에 번인**은 자막을 화면에 다시 씁니다. 서버가 줄을
SRT로 만들어 ffmpeg에 넘기면, ffmpeg가 원본 옆에 새 파일로 인코딩합니다. 원본은
건드리지 않고, 번인을 다시 하면 새 파일에 번호가 붙습니다(`x - mimiwatch (2).mp4`).
오디오는 그대로 복사하고 영상만 재인코딩합니다.

**로컬 파일만.** 전제는 이 기계에 영상 파일이 있어야 한다는 것인데, 그런 종류는
로컬 파일뿐입니다. 유튜브/트위치 녹화본은 전사용으로 오디오 렌디션을 가져오고
영상 자체는 내려받지 않으므로, 넣을 파일이 없습니다.

**ffmpeg에 `subtitles` 필터가 있어야 합니다.** 그 필터는 libass로 든 빌드에
있는데, 기본 Homebrew `ffmpeg`에는 없습니다. 시작 전에 묻습니다 — 번인을 못 하는
ffmpeg는 암호같은 종료 번호가 아니라 그대로 알려 줍니다.

영상 전체 인코딩이라 시간이 걸립니다. 대화상자가 닫히고 작업 상자가 진행을
따릅니다. 멈추면 파일은 남지 않습니다.

## 감시 (시작하면 알아서 받습니다)

주소를 서버에 남겨 두면 서버가 대신 확인합니다. 영상 목록의 **감시**를 열고
**＋ 감시**로 채널 또는 스트림 주소(와 이름)를 넣으십시오. 그 주소가 라이브가
되는 순간, 서버가 알아서 받기 시작합니다.

브라우저는 이것을 못 합니다. 탭을 닫으면 방송도 닫히고, 확장은 오래 살지
못합니다. 탭보다 오래 사는 것은 모델을 들고 있는 프로세스, 곧 서버입니다. 그래서
확인하는 쪽은 서버입니다.

시작하는 시점은 절제됩니다. 아무것도 받고 있지 않을 때만, 같은 주소의 다른
세션이 없을 때만요. 방송을 보고 있으면서 감시 대상이 라이브가 되면 기다립니다.
화면이 비면 다음 확인 때 시작합니다. 방송이 끝나고 다시 시작되면 다시
받습니다 — 그것이 감시의 쓸모입니다.

라이브 여부는 확인으로 압니다. 30초마다 yt-dlp 메타데이터 한 번. 채널 페이지면
녹화물 사이로 라이브가 있는지 보는 것이고, 스트림 주소면 라이브 표시를 그대로
읽습니다. 알 수 없으면 이전 결과를 유지합니다.

**목록이 비어 있어도 이 절은 그대로 있습니다.** 예전에는 비면 스스로 숨었는데,
첫 주소를 넣는 단 하나의 단추가 함께 숨었습니다. 그래서 이 기능은 아예 없는
기능과 구별되지 않았습니다.

제목 옆 머리글이 확인이 어떻게 되고 있는지 말해 줍니다. 마지막 확인이 몇 초 전에
끝났는지, 첫 확인이 끝나기 전이면 "확인하는 중…", 확인하는 스레드가 아예 없으면
"감시가 멎었습니다". 마지막 것은 다시 시작하면 풀립니다. 어느 것도 적어 두지
않습니다 — 돌고 있는 프로세스의 상태라서, 다시 시작하면 지워지고 첫 확인부터
다시 셉니다. 화면은 탭을 보고 있는 동안에만 15초마다 물어보고, 보고 있지 않으면
아예 묻지 않습니다.

각 주소는 **감시**(작은 원)가 있습니다. 껐다 켜다로 대기하고, 목록에서 언제든
지울 수 있습니다. 라이브인 줄을 누르면 수동으로 시작합니다 — 서버가 알아서
하는 것과 같은 일이며, 보고 있을 때 한다는 것만 다릅니다.
...
**경량 엔진에서도 정제 패스가 돌아간다.** 세그먼트 시각을 줄 수 없는 모델
(SenseVoice Small, Moonshine)은 발화 무리를 다시 받아 적어서 그 텍스트를 그
무리 자신의 VAD 경계 위에 앉힌다: 시간은 빠른 통과에서 그대로 지나오고, 텍스트
만 바뀐다. 그전엔 이 모델들은 패스를 건너뛰었기 때문에, 경량 쪽으로 전사한
녹화본은 빠른 통과 수준에서 머물렀다.
...
근거는 [`measurements/RESULTS.ko.md`](../measurements/RESULTS.ko.md)에 있습니다.
...
[docs/WINDOWS.ko.md](WINDOWS.ko.md)에 적어 두었습니다.
...
않습니다. 몇 달 지나면 **포맷 목록을 통째로 받지 못해** 라이브에서 "yt-dlp could not
find any audio at this URL." 가 됩니다 — 포맷이 없는 것이 아니라 아무것도
못 읽은 것입니다.
...
자세한 것은 [docs/PACKAGING.ko.md](PACKAGING.ko.md)에 있습니다.
...
머리의 **⧉ 따로 띄우기**를 누르면 세로로 긴 창이 하나 열립니다. 유튜브를 한쪽에
...
그대로 보고, 자막만 그 위에 뜹니다. 치지직에서도 돌고, 방송 페이지와 녹화본
페이지 어느 쪽이나 같습니다.
...
유튜브나 치지직 탭에서 확장 아이콘을 누르면 팝업이 뜹니다.
...
`자막 끔` / `시계 멈춤`. 어디를 봐야 할지가 거기서 갈립니다. 「화면에서 내리기」는 화면에서만 치웁니다 — 받아 적던 것을 끝내려면
mimiwatch 페이지에서 「● LIVE」 옆 ⋮ 안의 「중단」입니다. 같은 메뉴의
「자막 지우기」는 지금까지 쌓인 줄만 버리고 세션은 그대로 둡니다.
...
**⊕ 추가**에서 소스를 **"이 컴퓨터의 마이크"**로 고르고, 방 이름을 적고,
**소리를 파일로 저장을 켜고** 시작합니다. 브라우저가 마이크 권한을 묻고, 그 뒤로
두 가지가 동시에 일어납니다. 자막이 실시간 미리보기로 흐르고, **소리는
`data/recordings/<날짜>-<세션id>.wav`에 기록됩니다.**

두 번째를 일어나게 하는 것이 그 칸이고, 양식을 열면 꺼져 있습니다. 칸을 그대로
둔 마이크 세션은 전사만 하고 아무것도 남기지 않습니다 — 회의에서는 그것이 가장
피해야 할 결과이니, 방이 말을 시작하기 전에 확인할 단계입니다.

**중요한 것은 녹음이고 미리보기가 아닙니다.** 화자 구분은 녹화본 쪽 기능이라
라이브 세션은 누가 말했는지 말할 수 없는데, 회의 기록에서 필요한 것은 대부분
바로 그것입니다. 우회하는 방법은 세션이 WAV를 쓰게 두고, 회의가 끝나면 그
파일을 **☑ 화자 구분**을 켜서 새 작업으로 추가하는 것입니다. 그 패스는 파일
전체를 한 번에 받으므로 시작 시각만이 아닌 구간까지 얻고, 완결된 발화에 대한
정제 패스를 거치며, 실시간으로 따라올 수 있는 엔진이 아니라 설정된 엔진을
씁니다.

회의의 순서는 이렇습니다.

1. 마이크 소스로 녹음합니다. 미리보기는 소리가 들어오는지 확인하는 용도로만
   봅니다.
2. 끝나면 `data/recordings/<그 파일>.wav`를 화자 구분을 켜서 작업으로
   추가합니다.
3. 그 작업의 전사본을 읽습니다. 라이브 세션의 줄은 미리보기였습니다.

### 소리를 남기는 이유

실시간 전사는 도착하는 동안만 존재하는 것을 한 번 읽는 일입니다. 이 수신
경로를 이식해 온 hayamimi는 소리를 남기지 않는데, 그것으로 2026-09-11에 녹음한
회의는 그날 오후 CPU 전용 한국어 모델이 닿은 품질로 딱 한 번 전사되었습니다.
두 패스가 같은 구간에서 똑같이 뭉개졌고, 그래서 원인이 모델이 아니라 회의실
오디오라는 것이 갈렸습니다. 1차 확정본과 정제된 줄이 같은 자리에서 무너졌기
때문입니다. 그때 이미 더 나은 모델을 대 볼 대상이 남아 있지 않았습니다.
전사본은 소리에서 다시 만들 수 있지만, 소리는 전사본에서 다시 만들 수
없습니다.

파일을 쓰지 못하면 — 디스크가 찼거나 디렉터리를 만들 수 없으면 — 세션이 상태에
그 사실을 적고 **전사는 계속합니다.** 녹음을 잃는 것이 이미 화면에 있는 자막까지
잃게 해서는 안 됩니다. 맨 첫 블록에서 실패했다면 파일 자체가 남지 않습니다.
자동으로는 아무것도 지우지 않는 그 디렉터리에 빈 녹음이 남으면, 이유가 화면에서
사라진 뒤에도 실패한 녹음으로 읽히기 때문입니다.

### 서버가 받아 오는 방송을 저장하기

**⊕ 추가**에서 **소리를 파일로 저장**을 켜면 서버가 끌어오는 방송도 같은
디렉터리에 같은 형식으로 남습니다. 그 파일은 마이크로 만든 것과 세 가지가
다릅니다.

**시계가 아니라 이어 붙인 소리입니다.** 수신이 끊기면 세션은 ffmpeg를 다시 세워
붙습니다. 한 세션이 여러 ffmpeg에 걸칠 수 있다는 뜻이고, 파일을 ffmpeg의 두 번째
출력이 아니라 파이썬 쪽에서 쓰는 이유가 그것입니다 — 그 전부에 걸쳐 파일 하나로
남습니다. 파일을 닫는 것은 정지와 체크 해제 둘뿐입니다. 다시 붙는 일은 — 수신이
끊겼든 영상 저장을 바꿔 달라고 했든 — 열려 있는 파일을 건드리지 않고, 정지할 때
ffmpeg이 쥐고 있던 마지막 블록도 원래 있어야 할 그 파일로 들어갑니다. 도착하지
않은 소리는 그냥 파일에 없습니다. 빈자리를 무음으로 채우지 않고 붙여 버리므로
녹음은 방송보다 놓친 만큼 *짧고*, 얼마나 놓쳤는지는 자막에
적힙니다(`⋯ 수신이 끊긴 사이 약 N초를 받지 못했습니다 ⋯`). 실시간 길이에 맞춰
무음을 채우는 것은 먼저 재 보아야 할 변경이고, 아직 아무도 재지 않았습니다.

**이어받기는 파일을 하나 더 엽니다.** **이어받기**는 같은 세션을 이어갑니다 —
같은 id, 같은 자막 — 하지만 녹음은 새로 열고 시작한 시각으로 이름을 붙입니다.
한 번 이어받은 방송 하나는 WAV 두 개를 나란히 남깁니다.

**쓰기에 실패하면 로그가 아니라 화면에 뜹니다.** 실패해도 세션은 전사를 계속하므로
화면의 다른 어느 곳도 달라 보이지 않습니다. 대신 상태 줄이 호박색으로 바뀌며
*소리 저장이 멈췄습니다*라고 적습니다.

### 영상까지 저장하기

**영상도 파일로 저장**을 켜면 그림도 소리 옆에
`data/recordings/<날짜>-<세션id>.mp4`로 남습니다. 밤새 켜 두기 전에 알아 둘 것이
여섯 있습니다.

**방송은 한 번만 받아 와서 갈라 씁니다.** 칸을 켜면 그림이 이미 돌고 있던
ffmpeg의 두 번째 *입력*이 되고, 그 한 프로세스가 세 가지를 먹입니다 — 자막, (그
칸도 켰다면) WAV, 그리고 mp4. 그중 둘은 그 프로세스 자신의 출력입니다. 소리는
파이프로, 방송 자체는 파일로 나갑니다. WAV는 그 파이프에서 파이썬이 따로 쓰는데,
재접속을 건너 하나로 이어진 파일이 되는 것이 그 덕입니다. 두 번째 프로세스가 사본을
따로 받아 오는 방법도 있었지만, 한 방송에 스트림 대역폭을 두 배로 쓰고 조각 요청도
두 벌이 됩니다.

**그림은 더해진 입력이지 바뀐 입력이 아닙니다.** ffmpeg이 전사에 쓰는 소리는 칸을
끄고 있을 때 읽는 바로 그 소리만 있는 화질입니다. 그러니 칸을 켜도 VAD가 듣는 것도
WAV에 남는 것도 달라지지 않습니다 — 같은 표본, 같은 자막입니다. 유튜브에서는 그렇게
만들 수밖에 없기도 합니다. 도는 방송에는 두 스트림이 한 파일에 든 화질이 아예
없어서, 추출이 내주는 여덟 화질은 영상만 여섯과 소리만 둘이고, 미리 합쳐진 파일 하나를
달라고 하면 그대로 실패합니다(*Requested format is not available*) — 영상 저장이
거기서 매번 하던 일이 그것이었습니다. 두 스트림이 한 파일에 든 것을 내주는 사이트는
— 치지직 생방송의 화질은 전부 그 모양이고 소리만 있는 것은 하나도 없습니다 — 주소
하나로 돌아오고, 그때는 입력도 하나이며 소리는 예전처럼 그림이 든 화질에서 꺼냅니다.
멀티뷰는 세션을 넷까지 돌리므로 mp4도 넷까지 함께 자랄 수 있지만, 그것은 어차피
돌고 있던 넷이 픽셀을 버리는 대신 디스크까지 들고 가는 것입니다.

**한 번만 읽는 값은 출력끼리 운명을 같이한다는 것입니다.** 쓸 수 없는 mp4는 —
디스크가 찼거나 볼륨이 읽기 전용이 되었거나 — 자막을 먹여 주던 프로세스까지
끝냅니다. **저장이 실패하면 재접속에 걸리는 동안 수신이 끊긴다**는 뜻입니다(몇
초이고, 그 사이 못 받은 만큼은 `⋯ 수신이 끊긴 사이 약 N초를 받지 못했습니다 ⋯`로
자막에 적힙니다). 세션은 스스로 회복하며, 한도가 있습니다. 시작한 지 30초 안에 끝난
조각이 연달아 다섯이면 **그 세션에서는 영상을 포기합니다.** 상태 줄이 호박색으로
바뀌며 *영상 저장이 멈췄습니다*와 그 이유를 적습니다. 그 뒤로는 소리만 있는 화질로
돌아가 자막을 계속합니다. 그림을 아예 받아 오지 못하면 그 자리에서 소리만 있는
화질로 물러서고 같은 자리에 그 사실을 적습니다. 그림을 구하지 못했다고
자막까지 잃는 것은 잘못된 교환입니다.

**그림은 복사하고 소리는 다시 인코딩합니다.** `-c:v copy -c:a aac`이고, 소리 쪽은
고른 것이 아니라 어쩔 수 없는 것입니다. HLS는 MPEG-TS 조각을 내려 주고 그 안의
AAC는 ADTS 프레임으로 오는데, mp4는 그것을 받지 않습니다. 실제 HLS 방송을
`-c copy`로 그대로 복사하면 *Malformed AAC bitstream detected*로 즉시 죽고 재생되지
않는 조각만 남습니다. 흔한 해법인 `-bsf:a aac_adtstoasc`는 그 한 경우를 고치고
나머지를 망칩니다. Opus에서는 아예 시작조차 하지 못하고, AAC가 이미 ASC 형태인
CMAF 방송에서는 오디오 패킷을 전부 버리면서도 정상 종료합니다 — 멀쩡해 보이는데
소리가 없는 파일입니다. 소리를 다시 인코딩하면 한 명령으로 전부를 감당하고, 그쪽이
싼 쪽입니다. 4Mbps 영상 30초를 이 방식으로 다시 담는 데 CPU 0.17초가 들었고, 순수
복사는 0.03초였습니다. 비싼 쪽은 여전히 바이트 복사입니다.

**긴 방송은 파일 여러 개를 남깁니다.** 사이트가 내주는 주소는 서명되어 있고 하루
안에 만료되며 수신 자체도 끊기므로, 방송이 이어지는 중에 방송을 읽던 ffmpeg이
끝납니다. 다시 붙을 때마다 첫 파일 옆에 다음 조각을 엽니다 — `… (2).mp4`,
`… (3).mp4`. 소리 프레임을 이어 붙이듯 복사본을 파일 중간부터 이어 쓸 수는 없기
때문입니다. **이어받기**는 두 번째 WAV를 열듯이, 자기 시각으로 자기 조각들을
엽니다.

**중간에 잘린 파일도 재생됩니다.** fragmented mp4로
씁니다(`+frag_keyframe+empty_moov+default_base_moof`). 색인이 끝에 한 번이 아니라
파일이 자라는 동안 함께 내려간다는 뜻입니다. 세션을 멈춘 것이 아니라 mimiwatch가
죽은 경우 — 사람이 닫는 프로그램은 대개 그렇게 끝납니다 — 마지막 온전한 조각까지
재생되고, 잃는 것은 많아야 키프레임 한 간격이며, 고칠 것이 없습니다. 옆의 WAV는
정반대이고 위에 적은 복구가 필요합니다.

### 방송이 도는 중에 저장을 바꾸기

수신 중인 세션은 그 두 칸을 상태 줄에 스위치로 다시 답니다. 추가 화면은 방송이
시작되기 전에 묻는데, 방송에서는 그때가 이 질문에 답하기 가장 어려운 순간입니다.
남길 만한 일은 대개 방송이 시작된 다음에 벌어지기 때문입니다.

**소리 쪽을 바꾸는 값은 아무것도 아닙니다.** 바이트는 이미 기록기 앞을 지나가고
있으므로, 켜면 다음에 들어오는 덩어리에서 WAV를 열고 끄면 열려 있던 것을 닫습니다.
끊기는 것도, 다시 받아 오는 것도 없고, 스위치 양쪽에서 표본 하나도 새지 않습니다.
다시 켜면 앞 파일을 잇지 않고 *두 번째* 파일을 엽니다. **이어받기**와 같습니다.

**영상 쪽을 바꾸는 값은 재접속입니다.** 지금 방송을 읽고 있는 ffmpeg은 시작할 때
참이던 답에 맞춰 고른 입력과 출력 목록을 들고 있습니다 — 소리 하나에 출력
하나이거나, 그림을 두 번째 입력으로 더해 출력 둘이거나 — 그리고 도는 프로세스
아래에서는 둘 다 바꿀 수 없습니다. 그래서 스위치는 그것을 끝내고 다음 것을 바로
세웁니다. 방송은 몇 시간 동안 끊기지 않을 수도 있으니 자연히 끊길 때를 기다리지
않습니다. 기계
하나에서 도는 유튜브 방송으로 실측한 값으로, 주소를 다시 받아 오는 데
`yt-dlp -j` 1.6~2.1초에 `yt-dlp -g` 한 번 1.7~2.0초 — **3.5~4.2초**이고, 프로세스를
다시 세우는 시간과 첫 조각은 그 뒤에 붙습니다. 기계와 회선이 다르면 달라집니다.
숫자가 아니라 *몇 초*로 읽으십시오. 플레이어 위 안내 띠는 바꾸는 중이라고
정확히 그만큼만 적습니다. 새 프로세스가 실제로 읽기 시작하면 내려가고, 몇 초를
세어 내리지는 않습니다. 그렇게 하면 다시 받아 오는 데 오래 걸릴 때마다
수신이 돌아왔다고 거짓말을 하게 됩니다.

**플레이리스트가 맞춤 기준으로 쓸 시계를 가진 곳에서는 전사가 그것을 견딥니다.** 그럴
때 되감기 구간 안에 드는 끊김은 멈춘 자리에서 그대로 이어지므로, 보는 쪽에서는
자막이 잠시 뒤처졌다 따라붙는 것으로 보입니다. 유튜브와 트위치가 그 경우입니다.
그러나 제자리를 찾아 되감으려면 플레이리스트 앞에 놓인 조각이 방송의 어느 지점인지를
알아야 하고, 모든 사이트가 그것을 알려 주지는 않습니다. 치지직은 제 첫 조각보다
*늦은* 시작 시각을 내놓으므로 두 수를 서로 뺄 수가 없고, 생 m3u8은 둘 다 없을 수도
있습니다. 그럴 때는 라이브 끝에 붙어 다시 받기 시작하고, 끊긴 사이의 몇 초는 정말로
사라집니다. 구간을 아예 읽을 수 없는 경우와 같습니다. **어느 쪽이든 자막은 그 사실을
적고, 잰 것만 적습니다** — 길이를 아는 때는 `⋯ 저장을 바꾸는 사이 약 N초를 받지
못했습니다 ⋯`, 모를 때는 `⋯ 저장을 바꾸는 사이 방송의 일부를 받지 못했습니다.
이 사이트의 플레이리스트는 얼마만큼인지 알려 주지 않습니다 ⋯`입니다. 그 자리에
N을 적으면 직전 끊김의 숫자를 적는 셈이고, 예전에 실제로 그렇게 했습니다. 둘 다
저절로 끊겼을 때의 문장과 일부러 다르게 적습니다. 사용자가 요청한 끊김은 무언가가
거듭 실패하는 것을 지켜보는 두 횟수, 곧 재접속 횟수에도 영상을 포기시키는 조각
횟수에도 세지 않습니다.

**영상을 다시 켜는 것은 진짜 재시도입니다.** 영상을 포기하는 것은 그 세션에서
영구적이므로, 새벽 네 시에 화질을 한 번 받아 오지 못했거나 시작하자마자 끝난
조각이 연달아 다섯이면 밤새 도는 방송이 남은 시간 내내 그림을 한 장도 남기지
않습니다. 영상을 껐다 켜면 그것을 지우고 다시 시도합니다.

마이크·탭 세션에는 스위치가 둘이 아니라 하나입니다. 추가 화면에 칸이 하나인 것과
같은 이유로, 소리만 올려 보내므로 저장할 그림이 없습니다.

### 마이크 경로가 하지 않는 것

- **장치 선택 화면이 없습니다.** 시스템 기본 입력을 씁니다. 다른 것을 쓰려면
  브라우저의 사이트 설정에서 고릅니다.
- **브라우저의 보정을 끕니다** — 에코 제거, 노이즈 억제, 자동 게인. 이 셋은
  헤드셋을 가까이 댄 한 사람에 맞춰 조정된 것이고, 이 경로는 마이크 하나를
  두고 여러 사람이 떨어져 앉은 경우를 위한 것입니다. 그런 조건에서는 노이즈
  억제가 멀고 작은 목소리를 잡음으로 취급할 수 있고 자동 게인은 문장 안에서
  레벨을 움직입니다. 그 재료에서 보정을 끈 쪽이 더 잘 전사되는지는
  **측정되지 않았습니다.** 판단의 근거는 녹음이 보관본이라는 것입니다. 보정은
  원본에 나중에 적용할 수 있지만, 보정된 파일에서 다시 빼낼 수는 없습니다.
- **확장은 마이크 세션을 시작하거나 이어받지 않습니다.** 확장은 탭 오디오를
  잡고, 마이크는 페이지의 기능입니다.
- 세션을 멈춘 것이 아니라 프로세스가 죽은 경우에는 WAV의 길이 필드가 짧게
  남습니다. 그 필드는 닫을 때 쓰이기 때문입니다. 샘플은 전부 디스크에 있고
  `ffmpeg -i short.wav fixed.wav`가 헤더를 다시 씁니다.

## 문서뷰

자막은 "지금 무슨 말을 하고 있는가"에 답하고, 대본 패널은 "무슨 말을 했는가, 순서대로"에
답합니다. 둘 다 "그래서 이게 무슨 얘기였는가"에는 답하지 않는데, 세미나에서 끝나고 남는
질문은 사실 그것 하나뿐입니다.

컨트롤 줄의 **문서뷰**는 영상 자리에 문서를 얹습니다. 소제목이 있고 그 아래 몇 줄이
달리며, 발표가 진행되는 대로 자랍니다. 아래의 플레이어는 멈추지 않습니다 — 발표에서
중요한 것은 소리이지 화면에 떠 있는 슬라이드가 아니니, 읽는 동안에도 발표자는 계속
말합니다. 마이크 세션은 애초에 아래에 아무것도 없습니다.

전사본이 있는 자리라면 어디서나 켤 수 있습니다. 마이크 세션이든, 방송이든, 녹화본이든
마찬가지입니다. 값을 하는 자리가 발표이기 때문에, 토글은 메뉴가 아니라 전체화면 버튼
옆에 있습니다.

### 앞으로 쓰지, 다시 쓰지 않습니다

한 번 쓸 때 보는 것은 지금 쓰고 있는 절과, 지난번 이후 도착한 말뿐입니다. 그 위는 이미
굳어 있습니다.

다른 방법 — 매번 전사본 전체를 넘기고 요약을 새로 받는 것 — 은 만들기 전에 접었습니다.
자막을 번역하는 그 엔진 위에서 비용이 세션 길이의 제곱으로 늘고, 90초마다 통째로 다시
쓰이는 문단은 쓰이는 동안 읽을 수가 없습니다. 그래서 맨 아래 절 하나만 움직이고,
움직이는 동안 그렇다고 표시합니다.

소제목의 시각은 그 말이 나온 자리로 돌아가는 길입니다. 녹화본에서 누르면 플레이어가
그리로 갑니다. 정확도는 줄 단위가 아니라 1~2분 단위입니다. 한 번의 패스는 말 한 덩이를
받고 그 안 어딘가에서 새 주제가 시작됐다고 판단할 수는 있어도 어디인지는 모르는데,
모델에게 묻지 않기 때문입니다 — 물어서 얻는 답은 지어낸 답입니다.

### 요청할 때만 돕니다

한 번 쓰는 것은 자막을 번역하는 그 엔진을 한 번 통째로 쓰는 일이라, 아무도 문서를 열지
않은 세션은 아무 값도 치르지 않습니다. 라이브 세션에서는 **문서 쓰기 시작**을 한 번
누르면 그 뒤로 따라옵니다. 누르기 전의 말도 먼저 읽어 들이므로, 발표 30분째에 문서를
열어도 남은 부분이 아니라 그 30분을 받습니다.

녹화본은 이미 끝난 것이라 비용을 숨길 박자가 없습니다. 전사본 전체를 한 번에 읽는
작업으로 돌고, **문서 쓰기**가 그것을 시작합니다.

패스 간격은 일부러 벌려 두었습니다. 말이 1분쯤 쌓이기 전에는 아무것도 쓰지 않고, 그
뒤로도 1분에 한 번보다 자주 쓰지 않습니다.

### 쓸 수 있는 엔진이 있어야 합니다

경량 기본값인 M2M-100은 문장 하나를 다른 말로 옮길 뿐 그 밖의 일은 하지 못합니다. 요약
프롬프트를 주면 그 프롬프트를 번역해서 돌려줍니다 — 오류도 아니고 빈 결과도 아니고,
맞는 답처럼 읽히는 틀린 답입니다. 그래서 이 엔진에서는 문서가 아예 돌지 않고, 문서가
있어야 할 자리에 그 사실을 적습니다.

문서를 쓰려면 **⚙ 엔진**에서 Gemma나 OpenAI 호환 엔진을 고르세요. 채널 시어집도 패스에
함께 들어갑니다. 그것이 그 발표의 고유명사이고, 전사가 틀리는 것이 바로 그 단어들입니다.

### 남기기

**마크다운으로 저장**은 문서를 시각과 함께 파일로 씁니다. 문서는 전사본과 함께
보관되므로 다시 켠 뒤에도, 탭을 닫은 뒤에도 그대로 있고, 세션이나 녹화본을 지우면 같이
지워집니다.

**문서가 쓰이는 바탕은 실시간 미리보기입니다.** 위의 *회의와 세미나*에 적힌 것이 그대로
따라옵니다. 회의라면 더 온전한 기록은 여전히 끝난 뒤 화자 구분을 켜고 다시 전사한
WAV 쪽이고 — 문서는 그 전사본 위에 다시 쓸 수 있습니다. 그쪽이 더 나은 전사본입니다.

## 알려진 한계

**라이브 자막과 영상의 정렬은 수동입니다.** 오프셋 슬라이더로 맞추십시오.
값은 채널별로 기억해 두어, 같은 채널의 첫 방송만 손을 씁니다. 다음 방송은
맞춰져 있어 뜹니다. 녹화본은 자동으로 맞습니다.
...
않았습니다. 목소리가 하나인 방송은 화자 태그에 **1인 방송** 표시를 함께
넣으십시오. 나눠지는 것 자체가 오류이고, 둘 째 화자가 잡아 줄 사람도 없으니까
전부 한 사람으로 칩니다. 유사도 문턱도 같은 곳에 넣습니다 -- 낮게 하면 사람으로 더 많이
나누고, 높게 하면 하나로 모읍니다. 임베딩 모델이 디스크에서 사라졌다면 태그만
안 나와요. 전사본은 잃지 않습니다.
...
| `data/recordings/` | **소리를 파일로 저장**을 켰을 때 세션이 쓴 WAV — 소스와 무관하게, 켜지 않으면 남지 않습니다. 한 시간에 115MB쯤입니다(16kHz × 2바이트 = 초당 32KB). 그리고 **영상도 파일로 저장**을 켰을 때 서버가 받아 온 방송의 mp4 — 번호가 붙은 조각들로, 화질이 정하는 크기만큼. 자동으로 지우지 않습니다 — 다시 만들 수 없는 유일한 산출물입니다 |
