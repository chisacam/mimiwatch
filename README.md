...
전체화면에서 **자막 모드는 그대로 바꿀 수 있습니다.** 마우스를 움직이면
아래쪽에 조절기가 뜨고 잠시 뒤 사라집니다. 창에서 고른 것과 같은 값을
공유하므로 어느 쪽에서 바꾸든 함께 움직입니다.

글자 크기·배경·위치 슬라이더와 스크립트 패널은 전체화면 상자 밖이라
보이지 않습니다. 그것들을 만지려면 창으로 돌아오십시오.
