...
2. **라이브의 `Refiner` 를 녹화본에 그대로 옮기는 것은 잘못된 이식입니다**(49절).
   ...
   남아서 찾아가는 대상입니다.
3. **붙인다면 「정제 → 구간 시각으로 되쪼개기」입니다.** 런타임이 이미 내주는 값이고
   (`transcribe_cpp.Result.segments` 의 `t0_ms`/`t1_ms`), 어댑터에 손잡이를 냈습니다
   (`tcpp_asr.TranscribeCppASR.transcribe(segments=True)`, 기본 꺼짐 -- 라이브의 짧은
   조각까지 다른 해독 경로를 탈 이유가 없습니다). 구현은 아직 붙이지 않았습니다.
4. 표본이 하나(116분 애니메이션 영화)이고 정답이 팬자막입니다. 방송 표본으로 한 번 더
   재고 나서 기본값으로 삼는 것이 맞습니다.
