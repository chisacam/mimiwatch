...
저장하지 않고, 끄면 수신 쪽은 아무것도 달라지지 않습니다.
...
**방송은 한 번만 받아 와서 갈라 씁니다.** 칸을 켜면 mimiwatch가 읽는 화질이
muxed 쪽이 되고, 그 한 번의 읽기가 세 가지를 먹입니다 — 자막, (그 칸도 켰다면)
WAV, 그리고 mp4. 그중 둘은 ffmpeg 자신의 출력입니다. 소리는 파이프로, 방송 자체는
파일로 나갑니다. WAV는 그 파이프에서 파이썬이 따로 쓰는데, 재접속을 건너 하나로
이어진 파일이 되는 것이 그 덕입니다. 두 번째 프로세스가 muxed 사본을 따로 받아 오는
방법도 있었지만, 한 방송에 스트림 대역폭을 두 배로 쓰고 조각 요청도 두 벌이 됩니다.
칸을 끄면 수신 쪽은 아무것도 달라지지 않습니다 — 소리만 있는 화질, 같은 명령, 같은
자막입니다. 멀티뷰는 세션을 넷까지 돌리므로 mp4도 넷까지 함께 자랄 수 있지만,
그것은 어차피 돌고 있던 넷이 픽셀을 버리는 대신 디스크까지 들고 가는 것입니다.
...
돌아가 자막을 계속합니다. muxed 화질을 아예 받아 오지 못하면 그 자리에서 소리만
있는 화질로 물러서고 같은 자리에 그 사실을 적습니다. 그림을 구하지 못했다고
...
참이던 답에 맞춰 고른 화질과 출력 목록을 들고 있습니다 — 소리만 있는 화질에 출력
하나이거나, muxed 화질에 출력 둘이거나 — 그리고 도는 프로세스 아래에서는 둘 다
바꿀 수 없습니다. 그래서 스위치는 그것을 끝내고 다음 것을 바로 세웁니다. 방송은
몇 시간 동안 끊기지 않을 수도 있으니 자연히 끊길 때를 기다리지 않습니다. 기계
하나에서 도는 유튜브 방송으로 실측한 값으로, 주소를 다시 받아 오는 데
`yt-dlp -j` 1.6~2.1초에 `yt-dlp -f best -g` 1.7~2.0초 — **3.5~4.2초**이고, 프로세스를
...
영구적이므로, 새벽 네 시에 muxed 화질을 한 번 받아 오지 못했거나 시작하자마자 끝난
