...
**전사는 대개 그것을 견딥니다.** 되감기 구간 안에 드는 끊김은 멈춘 자리에서 그대로
이어지므로, 보는 쪽에서는 자막이 잠시 뒤처졌다 따라붙는 것으로 보입니다. 구간을
읽을 수 없는 경우에는 그 몇 초가 정말로 사라지고, 자막이 그 사실을 제 문장으로
적습니다 — `⋯ 저장을 바꾸는 사이 약 N초를 받지 못했습니다 ⋯`. 저절로 끊겼을 때의
문장과 일부러 다르게 적습니다. 사용자가 요청한 끊김은 무언가가 거듭 실패하는 것을
지켜보는 두 횟수, 곧 재접속 횟수에도 영상을 포기시키는 조각 횟수에도 세지 않습니다.
