...
조절기는 2.5초 뒤 사라집니다. **다시 부르려면 커서를 화면 아래쪽 가운데로
가져가십시오.** 영상 위의 마우스 움직임은 우리에게 오지 않습니다 — iframe
안은 교차 출처라 이벤트가 넘어오지 않기 때문입니다. 그래서 그 자리에만
받는 띠를 얹어 두었습니다. 유튜브 제어줄(맨 아래 60px)은 가리지 않도록
그보다 위(73~201px)에 둡니다.

조절기가 사라지면 **커서도 함께 감춰집니다.** 커서가 iframe 위에 있는 동안
그 모양은 유튜브 문서가 정하므로, 바깥에서 감추려면 잠깐 투명한 막을 덮어
커서를 가져오는 수밖에 없습니다. 대가가 하나 있습니다 — 커서가 감춰진
동안의 첫 클릭은 유튜브에 닿지 않고 막을 걷는 데 쓰입니다. 그다음부터는
평소대로입니다.
