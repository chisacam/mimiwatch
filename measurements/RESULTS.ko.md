...
계속 시도해도 소용없으므로, **연달아 다섯 번** 실패하면 그때는 처리를 중단합니다.
...
환청을 만드는데, 13절의 다양도 검사가 그것을 잡습니다 — 실제로 테스트에서
...
`domInteractive`가 4435ms인 것은 `<script src=".../iframe_api">`가 동기
...
목록 썸네일에는 `fetchpriority="low"`를 답니다. 14장이 같은 호스트로
몰리면 플레이어 임베드가 그 뒤에 줄을 섭니다.

## 38. 자동 판별로 켠 라이브는 한 줄도 번역되지 않았습니다

탭 오디오 수신을 붙이며 그 길이 끝까지 이어지는지 재는 테스트(`bench/tab_ingest.py`)를
썼는데, 전사는 멀쩡한데 번역이 0건으로 나왔습니다. 처음에는 테스트를 잘못
작성한 것으로 보였습니다 -- 그 녹음이 한국어라 옮길 것이 없었으니까요.
...
번역만 조용히 빠집니다. 그리고 우리가 쓰던 테스트는 대부분 언어를 못 박아
...
주소로 받는 세션에는 달지 않습니다. yt-dlp가 제목을 가져오고 이어받을 때
...
문턱을 올려도 **빈 구간으로 떨어진 것이 하나도 없습니다** — 4초 조각에서는 no_speech
...
- **Whisper 문턱·프롬프트·폴백 손잡이는 이 표본에서 아무것도 바꾸지 않았습니다.** 두
  표본은 값이 글자 하나도 안 달랐습니다 -- 문턱이 한 번도 발동하지 않았다는 뜻입니다.
...
- **정제본을 무리째 한 줄로 내보내면 끝단이 집니다**(29.3 → 27.7). 글자는 좋아지는데
  ...
  chrF가 39.6으로 똑같은 것이 그 증거입니다 -- 내용은 그대로고 시각만 뭉갠 것입니다.
...
- 치르는 대가는 전사 시간입니다. 되쪼개기 포함 17배속으로, 정제 없는 talk보다 30~50% 깁니다.
  녹화본에는 지연 제약이 없으므로 치를 만한 대가입니다.
...
대가로 포착률이 98.9 → 98.3으로 내려갑니다(기준선과 같은 값입니다). 잃은 0.6p보다
줄이 겹치지 않는 편이 낫다고 보고 조이는 쪽을 뒀습니다. chrF는 여전히 기준선보다
...
5. **경량 기본 전사기에서는 정제가 돌지 않습니다.** 모델이 구간 시각을 낼 수 있어야
   ...
   품질을 원할 때 Gemma를 고르라는 47절의 안내와 같은 자리입니다.
...
하나로 다시 봤습니다 -- 白上フブキ의 귀청소 ASMR 29분(`vuvdEqKK3KY`), 일본어. **정답
자막이 없으므로 CER·chrF는 못 냅니다.** 볼 수 있는 것은 구조와 유지율, 그리고 눈입니다.
...
- 속삭이는 구간에서 whisper의 구간 시각이 2초 격자에 붙습니다(`85.36~87.36`,
  `87.36~89.36`, ...). 시각을 못 짚을 때 런타임이 둥근 값을 내는 것이고, 자막이
  이웃을 침범하지는 않습니다.
- **화자 태그는 혼자 하는 방송에서 다섯을 만들었습니다**(S1 274 · S2 33 · 나머지 8).
  `speaker_id`의 docstring이 경고한 그대로입니다 -- CAM++는 구간에 목소리가 충분해야
  하는데 이 표본의 확정 구간은 중앙 3.4초입니다(녹화본이 12초로 자른다는 그 문서의
  전제가 대사가 짧게 끊기는 방송에서는 성립하지 않습니다). 정제가 태그를 물려받는
  방식의 문제가 아니라 태그 자체의 한계이고, 「화자 태그 붙이기」는 기본이 꺼져
  있습니다. 정답이 없는 표본에서 문턱을 만지는 것은 재는 것이 아니라 맞추는 것이라
  손대지 않았습니다.

**결론: 51절의 잠정을 풉니다.** 방송 표본에서도 정제(되쪼개기)가 자막을 낫게 합니다.

# 치지직 녹화본, yt-dlp 없이 (2026-09-14)

치지직 방송은 이미 나갔고 그것은 yt-dlp가 풀어 주므로, 녹화본
(`chzzk.naver.com/video/<n>`)도 경로만 다른 같은 주소일 줄 알았습니다. 아니었습니다.
yt-dlp 2026.08.19과 같은 날의 master로 쟀습니다.

## 53. yt-dlp는 우리에게 필요한 녹화본을 열지 못해, 치지직의 엔드포인트에서 풉니다

녹화본 셋을 표본으로 삼았습니다.

| 녹화본 | 모양 | 길이 | yt-dlp |
|---|---|---|---|
| `5242545` | 업로드, `vodStatus` `ABR_HLS` | 68초 | 엽니다 |
| `15050677` | 다시보기, `liveRewindPlaybackJson` | 1562초 | 엽니다(HLS) |
| `15186552` | 다시보기, `vodStatus` `ABR_HLS` | 25337초 | **`KeyError('sourceURL')`** |

**지나가는 고장이 아니라 구조입니다.** `ABR_HLS`는 yt-dlp를 DASH 경로로 보내고,
`yt_dlp/extractor/common.py`는 `initialization.attrib['sourceURL']`을 조건 없이
읽습니다. 치지직은 그 속성이 없는 `<Initialization range="0-76870"/>`를 쓰므로
추출 전체가 죽습니다 -- 같은 매니페스트 안에 쓸 만한 단일 파일 표현 셋이 앉아
있는데도 그렇습니다. 그 한 줄을 너그럽게 고쳐도 실패가 한 줄 아래
`KeyError('media')`로 옮겨 갈 뿐입니다. `<SegmentURL>` 항목도 바이트 범위이기
때문입니다. yt-dlp의 DASH 쪽을 제대로 손보는 일이지 우리가 들고 다닐 패치가
아니고, 그날 master에도 두 고침 다 없었습니다. (68초짜리 업로드도 `ABR_HLS`인데
yt-dlp가 열었습니다. 무엇이 다른지는 쫓지 않았습니다 -- 택한 길이 거기에 기대지
않기 때문입니다.) **시험용으로 받은 녹화본이 바로 yt-dlp가 못 여는 모양이었다**는
것이 판단을 굳힙니다.

그래서 녹화본은 웹 플레이어가 쓰는 그 엔드포인트 둘에서 풉니다(`chzzk.py`).
돌아오는 것이 `-j`가 주던 것보다 낫습니다 -- 진짜 썸네일과 채널 id.

**주소는 만료됩니다.** 다시보기 재생목록은 `hdntl=exp=<unix>`로 서명되어 있고
(잴 때 17시간 뒤), 단일 파일은 제 `_lsu_sa_` 토큰을 답니다. 그러므로 저장하지
않습니다 -- 틀기 직전에 페이지가 달라고 합니다. 라이브 쪽이 이미 가지고 있던
규칙입니다(`live.play_url`).

**중계하지 않습니다.** 미디어 서버 둘 다 `access-control-allow-origin: *`로
답하고, 단일 파일은 `accept-ranges: bytes`와 함께 `206`을 주므로, 브라우저가 어느
쪽이든 직접 틉니다.

**다시보기는 플레이어에게 라이브 끝단이 아니라 녹화본입니다.** 렌디션 다섯의 HLS
마스터이고 `EXT-X-ENDLIST`가 있으며, `EXTINF` 합이 1562.2초로 API의 `duration`
1562와 맞습니다 -- 길이 전체를 찾아 갈 수 있습니다.

**소리는 어느 렌디션이나 같고 화면은 다르므로**, 전사는 가장 싼 것을 가져옵니다.
일곱 시간 방송에서 144p 단일 파일이 565MB, 1080p가 26GB입니다. ffmpeg은 그
565MB 파일을 HTTP로 찾아 들어가 한 시간 지점의 16kHz 모노 PCM 10초를 **2.3초**에
뽑습니다 -- `moov` 아톰이 앞에 있어 가운데에 닿는 데 통째로 받을 것이 없습니다.

**조각을 여덟씩(`-N 8`).** 다시보기는 2초짜리 조각으로 된 HLS 재생목록이고, 그것을
하나씩 이어 받는 시간은 대역폭이 아니라 왕복입니다.

| 무엇의 오디오 | 조각 | 기본 | `-N 8` |
|---|---|---|---|
| 26분짜리 치지직 다시보기 | 782 | 188.4초 | **5.6초** |
| 유튜브 녹화본(오디오 파일 하나) | 없음 | 3.8초 | 3.5초 |

펼칠 것이 없는 자리에서는 값이 들지 않습니다. 치지직에만이 아니라 모든 내려받기에
붙이는 이유입니다.

**이 가지의 끝단**: `probe`가 0.2~0.3초에 답하고, `fetch_audio`는 `5242545`에서
68.1초, `15050677`에서 1563.2초의 오디오를 냈습니다(API는 1562이라고 했습니다).

## 54. 문서 한 패스의 값과, 프롬프트가 들어가는가

문서뷰(`outline.py`)는 자막을 번역하는 그 엔진 위에서 1~2분마다 생성을 한 번씩 통째로
돕니다. 그것이 감당할 만한지를 가르는 두 숫자가 모두 없었습니다. **이 파일의 Gemma
측정은 전부 문맥 3줄 이하에 자막 한 줄짜리**(47·20·28·29절)인데, 이 프롬프트는 그보다
두 자릿수 큽니다.

표본: `9zlrjY2Upxo`, 30분짜리 한국어 컨퍼런스 발표("Datadog을 활용한 AWS 서버리스
Observability"), 발화 235줄, 14,400자, 이미 전사돼 있는 것. 엔진: 프로세스 안의 Gemma 4
E4B q4_0, `device: auto`, 스레드 4, `n_ctx` 2048 — 배포되는 기본값 그대로입니다.
`bench/outline_cost.py`.

| | |
|---|---|
| 창 | 928자 → 30분에 **17패스**, 발표 **1.6분**에 한 번 |
| 패스당 | 중앙값 **3.6초**, 최소 0.8초, 최대 5.3초 |
| 발표 전체 | 엔진 시간 **61초** · 21개 절 |
| 프롬프트 | 1,533 ~ 2,161자 |

**프롬프트는 들어갑니다. 그것도 어림한 것보다 여유 있게 들어갑니다.** `window_chars`는
한 글자를 한 토큰으로 셉니다. 안전한 쪽으로 일부러 틀리게 잡은 것이라고 그 자리에
적혀 있습니다. 모델 자신의 토크나이저로 재 보니 최악의 프롬프트 — 창을 꽉 채우고 진행
중인 절도 꽉 채운, 전부 한국어인 것 — 이 **2,205자 → 1,051토큰, 글자당 0.48토큰**이었습니다.
답에 420토큰을 떼어 두고도 **2048 중 577토큰이 남습니다.** 이 표본에서 창은 두 배쯤
보수적입니다.

**그래도 그대로 둡니다.** 실패하는 방식이 조용한 잘림인 예산을, 한 모델 한 언어
하나로 넓힐 수는 없습니다. 일본어와 중국어는 재지 않았습니다. 나중에 올리는 일이
짐작이 아니라 근거 위의 결정이 되도록 숫자만 여기 적어 둡니다.

**값이 나가는 자리는 3.6초가 아니라 그 3.6초가 누구 것이냐입니다.**
`LocalGemma.generate`는 `LocalGemma.translate`와 같은 락을 잡습니다 — llama.cpp의 컨텍스트는
동시 호출을 견디지 못합니다. 그러니 한 패스는 아무 자막도 번역되지 않는 3.6초입니다.
20절의 중앙값 0.18초에 대면 한 패스 뒤에 스무 줄어치 번역이 밀려 서는 셈이고, 요구사항
N2는 끝에서 끝까지 3초를 말합니다. 잃는 것은 없지만(뒤에 큐가 빠집니다) 1~2분에 한 번
줄이 늦게 도착합니다.

문서가 **요청하기 전에는 꺼져 있는** 이유가 그것이고, `OUTLINE_GAP_S`가 더 잰 값이
아니라 75초인 이유도 그것입니다. 원격 엔진이 뚜렷하게 나은 자리이기도 합니다.
OpenAI 호환 엔드포인트는 여기서 락을 잡지 않으므로, 패스가 자막에서 가져가는 것이
아무것도 없습니다.

**거는 것은 시계가 아니라 창입니다.** 패스당 발표 1.6분이면 75초 바닥은 발표에서 거의
걸리지 않습니다. 75초 동안 두 문장이 오가는 한산한 방을 위해 있는 것입니다.

**프롬프트가 지키게 하지 못한 것 하나.** 패스마다 새 절은 많아야 하나라고 적어 두었는데
17패스 중 4번이 둘을 열었습니다. 30분에 21개 절이면 1.4분마다 소제목 하나로, 문서가
원하는 것보다 잘습니다. 여기서 쫓지 않았습니다. 갈라진 결과 자체는 읽을 만하고, 고칠
자리는 측정이 붙은 손잡이가 아니라 프롬프트 문장입니다.
