...
- 끊긴 라이브 세션의 수신 재개. 지금은 중단됨으로 표시만 하고 자막을 읽게 해 줍니다 (아래 8-4 참고)
- 녹화본 흐름의 `tcpp` 전사 엔진 연결. `backends.json`에 `tcpp-best`가 기본으로 잡혀 있지만 `asr.build()`에는 `openai`와 `local` 분기밖에 없어, 녹화본은 조용히 hayamimi 로컬로 갑니다 (라이브는 `RoutedASR`을 직접 쓰므로 해당 없음)
